...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    type LendingWallet = {
      lending(): {
        deposit(req: unknown): Promise<{ wait(): Promise<void>; hash: string; explorerUrl?: string }>;
      };
    };
    const w = wallet as unknown as LendingWallet;

    const depositReq: Record<string, unknown> = {
      token,
      amount: parsedAmount,
    };
    if (poolAddress) {
      depositReq.poolAddress = fromAddress(poolAddress);
    }

    const tx = await w.lending().deposit(depositReq);
    await tx.wait();
